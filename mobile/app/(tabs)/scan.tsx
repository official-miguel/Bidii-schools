    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <View style={{ height: 320, position: 'relative' }}>
        {permission?.granted ? (
              ...
              <View style={{ width: 200, height: 200 }}>
                {(['tl','tr','bl','br'] as const).map(c => (
                  <View key={c} style={{
                    ...
                    width:32, height:32, borderColor: colors.primary,
          ...
          <View style={{ flex:1, backgroundColor: colors.foreground, alignItems:'center', justifyContent:'center', gap: Spacing[3] }}>
            <QrCode size={48} color={colors.mutedForeground} />
            <Text style={{ color: colors.mutedForeground, textAlign:'center', paddingHorizontal: Spacing[6] }}>
              Camera permission needed for scanning
            </Text>
            <TouchableOpacity onPress={requestPermission} style={{ backgroundColor: colors.primary, paddingHorizontal: Spacing[4], paddingVertical: Spacing[2.5], borderRadius: Radius.button }}>
              <Text style={{ color: '#FFFFFF', fontWeight: Typography.fontWeight.semibold }}>Enable Camera</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        <ScrollView contentContainerStyle={{ padding: Spacing[4], gap: Spacing[3], paddingBottom: insets.bottom + Spacing[8] }}>
          ...
          {result.state === 'idle' && (
            <View style={{ alignItems:'center', paddingTop: Spacing[6], gap: Spacing[2] }}>
              <QrCode size={32} color={colors.mutedForeground} />
          ...
          {result.state === 'processing' && (
            <View style={{ alignItems:'center', paddingVertical: Spacing[8], gap: Spacing[3] }}>
              ...
              <Text style={{ color: colors.mutedForeground, fontSize: Typography.fontSize.sm }}>Processing scan…</Text>
            </View>
          )}

          {result.state === 'success' && (
            <View style={{ backgroundColor: colors.success, borderRadius: Radius.card, padding: Spacing[5], alignItems:'center', gap: Spacing[2], borderWidth:1, borderColor: colors.successForeground + '30' }}>
              <CheckCircle2 size={36} color={colors.successForeground} />
              ...
              {result.detail && (
                <Text style={{ fontSize: Typography.fontSize.sm, color: colors.mutedForeground }}>{result.detail}</Text>
              )}
          ...
          {result.state === 'error' && (
            <View style={{ backgroundColor: colors.destructive + '15', borderRadius: Radius.card, padding: Spacing[5], alignItems:'center', gap: Spacing[2], borderWidth:1, borderColor: colors.destructive + '30' }}>
              <AlertCircle size={36} color={colors.destructive} />
              <Text style={{ fontSize: Typography.fontSize.base, fontWeight: Typography.fontWeight.semibold, color: colors.destructive, textAlign:'center' }}>
                {result.message}
              </Text>
