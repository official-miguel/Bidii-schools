...
export default async function TeacherDisciplineCasePage({ params }: { params: { id: string } }) {
  ...
  if (!permissions.RECORDS_DISCIPLINE?.canView) {
    redirect("/teacher");
  }
  ...
  // Check if teacher has permission to manage this specific case
  // (for close case functionality - only if they can create records)
    ...
    <div>
      ...
      <DisciplineCaseClient
        record={{
          ...
          dateOfOffence: record.dateOfOffence.toISOString(),
