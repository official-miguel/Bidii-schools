...
function RankingConfigForm() {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-5 max-w-xl">
      ...
      <div className="rounded-xl bg-background border border-border px-4 py-3 text-sm text-slate leading-relaxed">
        These three weights must sum to <strong className="text-foreground">1.0</strong>.
...
function LibrarySettingsForm() {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-8 max-w-2xl">
      ...
      {/* Borrowing rules */}
      <div>
        <h3 className="text-sm font-semibold text-foreground mb-1">Borrowing Rules</h3>
        <p className="text-xs text-slate mb-4">Controls how many books students can borrow, how long, and what fines accrue.</p>
      ...
      <div>
        <h3 className="text-sm font-semibold text-foreground mb-1">Eligibility & Card Expiry</h3>
        <p className="text-xs text-slate mb-4">Control which students are eligible for a library card and whether cards expire.</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <p className="text-xs text-slate mt-1">Leave blank to include all forms.</p>
          </div>
          <div>
            ...
            <p className="text-xs text-slate mt-1">Leave blank for no expiry.</p>
          </div>
          <div>
            ...
            <p className="text-xs text-slate mt-1">Flag books overdue by this many days.</p>
          </div>
        </div>
      </div>
      ...
      {displayUrl ? (
        ...
        <div className="mt-1 flex items-center gap-4">
          ...
          <img
            src={displayUrl}
            alt={`${label} preview`}
            className="h-20 w-20 object-contain rounded-xl border border-border bg-card p-1.5"
        ...
        <div
          ...
          className={`mt-1 flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed
            ...
            ${uploading
              ...
              : "border-border hover:border-teal/50 hover:bg-teal/3 dark:hover:border-teal/40"
            }`}
        >
          {uploading ? (
            ...
            <div className="flex flex-col items-center gap-1 text-center pointer-events-none">
              ...
              <p className="text-xs text-slate">PNG, JPG, WebP or SVG · max 2 MB</p>
            </div>
          )}
        </div>
      )}
    ...
    <form onSubmit={handleSave} className="space-y-8 max-w-2xl">
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="flex items-start gap-3 p-5 border-b border-border bg-slate-50/60/20">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground">Identity &amp; Branding</p>
            <p className="text-xs text-slate mt-0.5">Logo, stamp, and school motto</p>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="flex items-start gap-3 p-5 border-b border-border bg-slate-50/60/20">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground">Admission Policy</p>
            <p className="text-xs text-slate mt-0.5">Gender policy and boarding type govern student registration defaults</p>
          </div>
        </div>
      ...
      {boardingType !== "DAY_ONLY" && (
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          <div className="flex items-start gap-3 p-5 border-b border-border bg-slate-50/60/20">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground">Dormitory Auto-Allocation</p>
              <p className="text-xs text-slate mt-0.5">Automatically assign dorms on student registration</p>
            </div>
          </div>
          <div className="px-5 py-5">
            <label className="flex items-start justify-between gap-4 cursor-pointer">
              <div className="min-w-0">
                <p className="text-sm font-medium text-foreground">Enable auto-allocation</p>
      ...
      <div className="max-w-3xl space-y-5">
        ...
        <div className={`inline-flex items-center gap-2 text-sm font-medium rounded-lg px-3 py-2 ${
          isCustom
            ...
            : "bg-line/60 text-slate border border-border/40"
        }`}>
          {isCustom
          ...
          <table className="w-full text-sm">
            <thead className="bg-background">
              <tr>
                {["Band", "Min %", "Max %", "Points", "Achievement Level", "Description"].map((h) => (
                  <th key={h} className="px-3 py-2.5 text-left text-xs font-semibold text-slate">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-border ">
              {[...activeBands]
                ...
                .map((band) => (
                  <tr key={band.id} className="bg-card hover:bg-background/50/50 transition-colors">
                    <td className="px-3 py-2.5 font-semibold text-foreground">{band.bandName}</td>
                    <td className="px-3 py-2.5 text-slate tabular-nums">{band.minPercentage}%</td>
                    <td className="px-3 py-2.5 text-slate tabular-nums">{band.maxPercentage}%</td>
                    <td className="px-3 py-2.5 text-slate tabular-nums">{band.points}</td>
                    <td className="px-3 py-2.5 text-slate tabular-nums">{band.achievementLevel}</td>
                    <td className="px-3 py-2.5 text-slate">{band.description}</td>
                  </tr>
                ))}
            </tbody>
          </table>
        ...
        <table className="w-full text-sm min-w-[700px]">
          <thead className="bg-background">
            <tr>
              {["Band name", "Min %", "Max %", "Points", "AL", "Description", ""].map((h) => (
                <th key={h} className="px-3 py-2.5 text-left text-xs font-semibold text-slate whitespace-nowrap">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-border ">
            {draft.map((band, idx) => {
              ...
              return (
                <tr key={band._key} className="bg-card">
                  {/* Band name */}
