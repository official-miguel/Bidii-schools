...
function TrendChart({ trend }: { trend: TrendBucket[] }) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 shadow-xs">
      <h2 className="text-sm font-semibold text-foreground mb-4">Errors by Severity (last 14 days)</h2>
      <div className="flex items-end gap-3 h-24">
        {trend.length === 0 ? (
          <p className="text-xs text-slate self-center w-full text-center">No data</p>
        ) : (
          trend.map((t, i) => {
            const pct = (t._count.id / totalMax) * 100;
            return (
              <div key={i} className="flex flex-col items-center gap-1 flex-1 min-w-0">
                <span className="text-[10px] text-slate tabular-nums">{t._count.id}</span>
                <div
                  className={`w-full rounded-t-sm transition-all ${SEV_COLORS[t.severity] ?? "bg-slate-300"}`}
                  style={{ height: `${Math.max(pct, 4)}%` }}
                  title={`${t.severity}: ${t._count.id}`}
                />
                <span className="text-[9px] text-slate truncate w-full text-center">
...
function ErrorDrawer({
  ...
  const STATUS_BTNS: { value: string; label: string; cls: string }[] = [
    ...
    { value: "IGNORED",      label: "Ignored",      cls: "bg-slate-100 text-slate border-border hover:bg-slate-200"          },
  ];
  ...
  return (
    <>
      ...
      <aside
        ...
        className="fixed right-0 top-0 h-full w-full max-w-xl z-40 bg-card
                   border-l border-border shadow-xl overflow-y-auto flex flex-col
        ...
        <div className="flex items-start justify-between gap-4 px-6 py-5 border-b border-border">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {error.module && (
                <span className="font-mono text-[10px] text-slate bg-slate-100 px-1.5 py-0.5 rounded">
                  {error.module}
                </span>
              )}
            </div>
          ...
          <button type="button" onClick={onClose}
            className="shrink-0 flex items-center justify-center h-8 w-8 rounded-lg text-slate hover:bg-slate-100 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <dl className="grid grid-cols-2 gap-3 text-xs">
            {[
              ...
              <div key={label} className="rounded-lg bg-background border border-border px-3 py-2.5">
                <p className="text-slate font-medium uppercase tracking-wide text-[10px] mb-1">{label}</p>
          ...
          {error.stackTrace && (
            <div>
              <p className="text-xs font-semibold text-slate uppercase tracking-wide mb-2">Stack Trace</p>
              <pre className="text-[10px] leading-relaxed text-foreground bg-background
                              border border-border rounded-lg p-3 overflow-x-auto whitespace-pre-wrap break-all">
                {error.stackTrace}
              </pre>
            </div>
          )}
          ...
          {error.context && (
            <div>
              <p className="text-xs font-semibold text-slate uppercase tracking-wide mb-2">Request Context</p>
              <pre className="text-[10px] leading-relaxed text-foreground bg-background
                              border border-border rounded-lg p-3 overflow-x-auto whitespace-pre-wrap break-all">
          ...
          {/* Notes */}
          <div>
            <p className="text-xs font-semibold text-slate uppercase tracking-wide mb-2">Notes</p>
            <textarea
              value={notes}
              ...
              className="w-full rounded-lg border border-border bg-card
                         px-3 py-2.5 text-sm text-foreground placeholder:text-slate-light
...
export default function ErrorsPage() {
  ...
  const th = "px-5 py-3.5 text-left text-xs font-semibold text-slate uppercase tracking-wide";
  const thSortable = `${th} cursor-pointer hover:text-foreground select-none`;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <input type="text" value={fModule} onChange={e => setFModule(e.target.value)}
          ...
          className="rounded-xl border border-border bg-card
                     px-3.5 py-2.5 text-sm text-foreground placeholder:text-slate-light
                     focus:outline-none focus:border-teal focus:ring-2 focus:ring-teal/15 shadow-xs" />
                ...
                {sorted.map(row => (
                  ...
                  >
                    <td className="px-5 py-3.5 max-w-[260px]">
                      <p className="text-sm text-foreground truncate">{row.message}</p>
                    </td>
                    <td className="px-5 py-3.5 hidden sm:table-cell text-xs text-slate whitespace-nowrap">
                      {row.school?.name ?? "—"}
                    </td>
                    <td className="px-5 py-3.5"><SeverityBadge s={row.severity} /></td>
                    <td className="px-5 py-3.5 hidden md:table-cell text-xs font-mono text-slate">
                      {row.module ?? "—"}
                    </td>
                    <td className="px-5 py-3.5 text-xs text-slate whitespace-nowrap">
                    ...
                    <td className="px-5 py-3.5 hidden lg:table-cell text-sm tabular-nums text-slate">
                      {row.occurrences}
                    </td>
                    <td className="px-5 py-3.5">
                      <Eye className="h-4 w-4 text-slate" aria-hidden />
                    </td>
                  </tr>
                ))}
              </tbody>
