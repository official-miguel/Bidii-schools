...
// ─── Directory tab ─────────────────────────────────────────────────────────────
function DirectoryTab({ canAssignRoles = true }: { canAssignRoles?: boolean }) {
                ...
                <div>
                  ...
                  {!canAssignRoles ? (
                    <p className="text-xs text-slate mt-1.5">
                      Only the Principal can put a staff member on an administrative
                      role, since the role decides what they can reach.
                    </p>
                  ) : !isTeaching && (
                    <p className="text-xs text-slate mt-1.5">
...
/**
 * StaffWorkspace — register teaching and non-teaching staff, assign roles,
 * and control what each can access.
 *
 * This is the Principal's screen, lifted out of /principal/staff so the staff
 * portal can render the same one for anyone granted the STAFF module, rather
 * than the read-only card wall that used to stand in for it.
 *
 * `showRoles` gates the Roles & Permissions tab, which stays with the
 * Principal: whoever can edit a role can grant themselves every other module
 * through it, so delegating staff management must not hand over role editing
 * as well. The API behind that tab is Principal-only regardless — this keeps
 * the UI from offering something the server would refuse.
 */
export default function StaffWorkspace({
  ...
  contextLinks?: ContextNavItem[];
  showRoles?: boolean;
} = {}) {
  const [tab, setTab] = useState<"directory" | "roles">("directory");
  const activeTab = showRoles ? tab : "directory";

  return (
    <div>
      <ContextNavigation items={contextLinks} />
      ...
      {showRoles && <TabBar active={activeTab} onChange={setTab} />}
      {activeTab === "directory" ? <DirectoryTab canAssignRoles={showRoles} /> : <RolesTab />}
    </div>
  );
}
